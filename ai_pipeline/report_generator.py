...
def generate_report(results, passed, failed, skipped, history):
    ...
    base_dir = os.getenv("RESULTS_PATH", "test-results")
    report_dir = os.path.join(base_dir, "flakiness-report")

    os.makedirs(report_dir, exist_ok=True)

    # ✅ FIXED TestNG relative path
    testng_link = "../test-report/index.html"
    ...
    <ul>
        ...
        <li><a href="flaky.html">Flaky Tests</a></li>
    </ul>
    ...
    <table border="1">
    <tr><th>Test</th><th>Status</th><th>Flakiness</th></tr>
    """
    ...
    for r in results:
        ...
        <tr>
            <td>{r['test_name']}</td>
            <td>{r['status']}</td>
            <td>{r['flakiness_label']}</td>
        </tr>
        """

    tests_html += "</table></body></html>"
    ...
    <h1>Flaky Tests</h1>
    <table border="1">
    <tr><th>Test</th><th>Score</th><th>Failures</th></tr>
    """

    for test, data in history.items():
        if data.get("flakiness_label") in ["MEDIUM", "HIGH"]:
            flaky_html += f"""
            <tr>
                <td>{test}</td>
                <td>{data.get('flakiness_score')}</td>
    ...
    flaky_html += "</table></body></html>"

    # ---------------- WRITE FILES ----------------
    with open(os.path.join(report_dir, "index.html"), "w") as f:
        f.write(index_html)

    with open(os.path.join(report_dir, "tests.html"), "w") as f:
        f.write(tests_html)

    with open(os.path.join(report_dir, "flaky.html"), "w") as f:
        f.write(flaky_html)
